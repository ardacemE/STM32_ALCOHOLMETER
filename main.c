...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
#define sensorDigital GPIO_PIN_10
#define LED GPIO_PIN_5
#define buzzer GPIO_PIN_6
#define sensorAnalog GPIO_PIN_1
...
void setup() {
    ...
    GPIO_InitStruct.Pin = sensorDigital;
    ...
    GPIO_InitStruct.Pin = sensorAnalog;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  	  	    ...
	  	  	    bool digital = HAL_GPIO_ReadPin(GPIOA, sensorDigital);
	  	  	    HAL_ADC_Start(&hadc1);
	  	  		HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
	  	  	    analog = HAL_ADC_GetValue(&hadc1);

	  	  	    char buffer[50];
	  	  	    sprintf(buffer, "Analog value : %d\tDigital value : %d\n", analog, digital);
	  	  	    HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);
	  	  	    HAL_Delay(1000);

						if (analog >= 3500) {
							char buffer2[50];
							sprintf(buffer2, "Ehliyet gider.\n");
